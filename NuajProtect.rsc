...
# 3) "NuajP-DNS-Servers" includes safe DNS servers:
#    - Primary: safedns.allover.co.za, safedns2.allover.co.za (secure DNS, default)
#    - Optional: Cloudflare 1.1.1.1 for Families (malware or malware+adult filtering)
#    See DNS section comments for how to choose, delete, or enable/disable.
#
# Blacklist Sources:
# - DShield, SpamHaus, VOIP, Bruteforce, CINSscore, UHB
#
...
/ipv6 firewall address-list remove [find where list="NuajP-DNS-Servers"]

# Configure Router DNS to Use Safe DNS Servers
#--------------------------------------------------------------
# Sets safedns.allover.co.za (88.198.70.38, 88.198.70.39) as primary DNS servers
# IPv6 DNS servers included for compatibility
/ip dns set servers=88.198.70.38,88.198.70.39 allow-remote-requests=yes
/ipv6 dns set servers=2a01:4f8:140:5021::38,2a01:4f8:140:5021::39

# Add DNS Servers to NuajP-DNS-Servers
#--------------------------------------------------------------
# Purpose: Allow DNS traffic (TCP/UDP, ports 53, 5353) to these servers from WAN
# Default: safedns.allover.co.za and safedns2.allover.co.za are enabled
# Optional: Cloudflare 1.1.1.1 for Families servers are disabled by default
# How to Choose:
# - Keep only the DNS servers you want in this list
# - To use Cloudflare, enable by setting 'disabled=no' (e.g., /ip firewall address-list set [find address=1.1.1.2] disabled=no)
# - To use Cloudflare exclusively, remove safedns entries and update /ip dns set servers=1.1.1.2,1.0.0.2
# How to Delete: Remove unwanted entries (e.g., /ip firewall address-list remove [find address=1.1.1.2])
# How to Disable: Set 'disabled=yes' (e.g., /ip firewall address-list set [find address=88.198.70.38] disabled=yes)
/ip firewall address-list add list=NuajP-DNS-Servers address=88.198.70.38 comment="safedns.allover.co.za - Secure DNS, primary server (IPv4)"
/ip firewall address-list add list=NuajP-DNS-Servers address=88.198.70.39 comment="safedns2.allover.co.za - Secure DNS, secondary server (IPv4)"
/ip firewall address-list add list=NuajP-DNS-Servers address=1.1.1.2 disabled=yes comment="Cloudflare 1.1.1.1 for Families - Blocks malware (IPv4, disabled by default)"
/ip firewall address-list add list=NuajP-DNS-Servers address=1.0.0.2 disabled=yes comment="Cloudflare 1.1.1.1 for Families - Blocks malware (IPv4, disabled by default)"
...
/ipv6 firewall address-list add list=NuajP-DNS-Servers address=2a01:4f8:140:5021::38 comment="safedns.allover.co.za - Secure DNS, primary server (IPv6)"
/ipv6 firewall address-list add list=NuajP-DNS-Servers address=2a01:4f8:140:5021::39 comment="safedns2.allover.co.za - Secure DNS, secondary server (IPv6)"
...
# Static Blacklist: DShield (Unchanged as per your request)
#--------------------------------------------------------------
...
# IPv6 - Define Dynamic Blacklists (TCP)
#----------------------------------------------------------------
/ipv6 firewall filter remove [find where comment~"^NuajProtect"]
/ipv6 firewall raw remove [find where comment~"^NuajProtect"]

/ipv6 firewall filter add chain=input action=accept connection-state=established,related,untracked comment="NuajProtect - Accept established connection"
/ipv6 firewall filter add chain=input action=drop connection-state=invalid comment="NuajProtect - Drop invalid"
/ipv6 firewall filter add chain=input action=accept protocol=icmp comment="NuajProtect - Accept pings"
/ipv6 firewall filter add chain=forward action=accept ipsec-policy=in,ipsec comment="NuajProtect - Accept IPsec in"
/ipv6 firewall filter add chain=forward action=accept ipsec-policy=out,ipsec comment="NuajProtect - Accept IPsec out"
/ipv6 firewall filter add chain=forward action=accept connection-state=established,related,untracked comment="NuajProtect - Accept established"
/ipv6 firewall filter add chain=forward action=drop connection-state=invalid comment="NuajProtect - Drop invalid"
...
/ip firewall filter add chain=detect-ddos action=add-dst-to-address-list address-list=ddos-targets address-list-timeout=10m comment="NuajProtect - DDoS"
/ip firewall filter add chain=detect-ddos action=add-src-to-address-list address-list=ddos-attackers address-list-timeout=10m comment="NuajProtect - DDoS"
...
/ip firewall raw add chain=prerouting action=add-src-to-address-list in-interface-list=WAN dst-port=7,9,13,17,19,22,23,139,162,389,445,1433,3306,8291 protocol=udp src-address-list=NuajP-Dynamic-Blacklist address-list=NuajP-Dynamic-Blacklist address-list-timeout=6d log-prefix="NuajP - Blacklisted>" comment="NuajProtect - Blacklisted for 6 days Reset"
/ip firewall raw add chain=prerouting action=add-src-to-address-list in-interface-list=WAN dst-port=7,9,13,17,19,22,23,139,162,389,445,1433,3306,8291 protocol=udp src-address-list=NuajP-Dynamic-Strike3 address-list=NuajP-Dynamic-Blacklist address-list-timeout=5d log-prefix="NuajP - Blacklisted>" comment="NuajProtect - Blacklisted for 5 days"
/ip firewall raw add chain=prerouting action=add-src-to-address-list in-interface-list=WAN dst-port=7,9,13,17,19,22,23,139,162,389,445,1433,3306,8291 protocol=udp src-address-list=NuajP-Dynamic-Strike2 address-list=NuajP-Dynamic-Strike3 address-list-timeout=10m log-prefix="NuajP - Strike 3>" comment="NuajProtect - Strike 3"
/ip firewall raw add chain=prerouting action=add-src-to-address-list in-interface-list=WAN dst-port=7,9,13,17,19,22,23,139,162,389,445,1433,3306,8291 protocol=udp src-address-list=NuajP-Dynamic-Strike1 address-list=NuajP-Dynamic-Strike2 address-list-timeout=10m log-prefix="NuajP - Strike 2>" comment="NuajProtect - Strike 2"
/ip firewall raw add chain=prerouting action=add-src-to-address-list in-interface-list=WAN dst-port=7,9,13,17,19,22,23,139,162,389,445,1433,3306,8291 protocol=udp src-address-list=!NuajP-Dynamic-Strike1 address-list=NuajP-Dynamic-Strike1 address-list-timeout=10m log-prefix="NuajP - Strike 1>" comment="NuajProtect - Strike 1"
